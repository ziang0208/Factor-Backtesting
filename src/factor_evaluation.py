...
class BacktestAnalyzer:
    def __init__(self, start_date, end_date, trade_date, factor_path, ret_path, ud_path, group, index_file):
    ...
    def prepare_data(self):
        ...
        # 读取 stock_index DataFrame
        stock_index = pd.read_csv(self.index_file, header=None)
        csi500_stocks = set(stock_index.iloc[:, 1])
        ...
        for file in tqdm(factor_files):
            ...
            if  file in ret_files and date_tmp >= self.start_date and date_tmp <= self.end_date:
                ...
                # 使用 CSI500 指数筛选 final
                final = final[final.index.isin(csi500_stocks)]
