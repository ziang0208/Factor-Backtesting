...
#         return self.result, self.pnl, self.pro, self.ic
...
class FactorEvaluator:
    ...
    def calculate_effectiveness(self):
        for factor in self.ic:
            ...
            new_row = {'factor_name': factor, 'ic': ic, 'icir': icir, 'long_ret': long_ret, 'long_std': long_std, 'long_ir': long_ir, 'long_maxdd': long_maxdd,
                       'hedge_ret': hedge_ret, 'hedge_std': hedge_std, 'hedge_ir': hedge_ir, 'hedge_maxdd': hedge_maxdd}
            self.result = pd.concat([self.result, pd.DataFrame([new_row])], ignore_index=True)
